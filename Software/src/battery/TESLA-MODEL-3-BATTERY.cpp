...
bool forceOpenContactors = false;
...
int16_t max_temp = 0;   // C*
int16_t min_temp = 0;   // C*
...
uint16_t nominal_full_pack_energy = 600;
uint16_t bat_beginning_of_life = 600;
...
uint16_t soc_vi = 0;
uint16_t soc_ave = 0;
uint16_t cell_max_v = 3700;
uint16_t cell_min_v = 3700;
uint16_t cell_deviation_mV = 0;  //contains the deviation between highest and lowest cell in mV
uint8_t max_vno = 0;
uint8_t min_vno = 0;
...
the first, for a few cycles, then stop all  messages which causes the contactor to open. */
  ...
  if (currentMillis - previousMillis30 >= interval30) {
    ...
    if (inverterAllowsContactorClosing == 1) {
      if (system_bms_status == ACTIVE && !forceOpenContactors) {
        send221still = 50;
